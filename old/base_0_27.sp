...
// Plugin cvars
ConVar g_cvarAutoEnable;			// Automatically enable the plugin's features
ConVar g_cvarEnabled;				// The plugin's features are active
...
ConVar g_cvarUseSQL;				// Use SQLite store store points and prefs
ConVar g_cvarBlastPushing;			// Control blast force pushing, e.g. rocket jumping
ConVar g_cvarBuildings;				// Restrict engineer building types for both teams
ConVar g_cvarAllowEurekaEffect;		// Allow the Eureka Effect. If not, it will be replaced with a wrench.
ConVar g_cvarShowRoundStartMessage;	// Shows the round start messages
ConVar g_cvarUseActivatorHealthBar;	// Use the boss health bar for the blue player when they are damaged by other players.
...
public void OnPluginStart()
{
	...
	g_cvarAutoEnable			= CreateConVar("dtk_autoenable", "1", "Automatically enable the plugin when a deathrun map is detected.", _, true, 0.0, true, 1.0);
	g_cvarEnabled				= CreateConVar("dtk_enabled", "0", "Enable the plugin's features.", _, true, 0.0, true, 1.0);
	g_cvarPreventBlueEscape 	= CreateConVar("dtk_preventblueescape", "1", "Prevent the blue player from escaping their role during a round by suicide or switching team.", _, true, 0.0, true, 1.0);
	g_cvarRedSpeed 				= CreateConVar("dtk_redspeed", "0", "Set the speed red players should run in units per second. 0 is off.", _, true, 0.0, true, 750.0);
	g_cvarRedScoutSpeed 		= CreateConVar("dtk_redscoutspeed", "320", "Cap red Scout speed to this value. Ignored if dtk_redspeed is set. 0 is off.", _, true, 0.0, true, 750.0);
	g_cvarBlueSpeed 			= CreateConVar("dtk_bluespeed", "420", "Set blue run speed in units per second. 0 is off.", _, true, 0.0, true, 750.0);
	g_cvarRedScoutDoubleJump 	= CreateConVar("dtk_redscoutdoublejump", "0", "Allow red scouts to double jump. 0 = NO. 1 = YES.", _, true, 0.0, true, 1.0);
	g_cvarActivators 			= CreateConVar("dtk_activators", "1", "Number of activators (players on blue team). Setting to 0 will cause continual round resets on maps with a tf_logic_arena.", _, true, 0.0, true, 15.0);
	g_cvarTeamMatePush 			= CreateConVar("dtk_teampush", "0", "Team mate pushing", _, true, 0.0, true, 1.0);
	g_cvarRedDemoChargeLimits 	= CreateConVar("dtk_reddemocharge", "2", "Place limits on red demo charges. 0 = No charging. 1 = Regular behaviour. 2 = Limit duration and regeneration.", _, true, 0.0, true, 2.0);
	g_cvarRedSpyCloakLimits 	= CreateConVar("dtk_redspycloak", "2", "Place limits on red spy cloaking. 1 = Regular behaviour. 2 = Limit duration and regeneration.", _, true, 0.0, true, 2.0);
	g_cvarUsePointsSystem 		= CreateConVar("dtk_usepoints", "1", "Use the new points system for activator selection.", _, true, 0.0, true, 1.0);
	g_cvarTeamPushOnRemaining 	= CreateConVar("dtk_teampush_remaining", "5", "Number of players remaining on Red to activate team pushing. 0 = off.", _, true, 0.0, true, 30.0);
	g_cvarRedGlowRemaining 		= CreateConVar("dtk_redglow_remaining", "3", "Number of players remaining on Red to activate glow outline. 0 = off.", _, true, 0.0, true, 30.0);
	g_cvarRedGlow 				= CreateConVar("dtk_redglow", "0", "Make red players have a glowing outline for better visibility in maps with poorly designed activator corridors. 1 = on, 0 = off.", _, true, 0.0, true, 1.0);
	g_cvarRedMeleeOnly 			= CreateConVar("dtk_redmeleeonly", "0", "Restrict red players to melee weapons.", _, true, 0.0, true, 1.0);
	g_cvarChatKillFeed 			= CreateConVar("dtk_chatkillfeed", "1", "Show clients the classname and targetname of the entity that killed them.", _, true, 0.0, true, 1.0);
	g_cvarUseSQL 				= CreateConVar("dtk_usesql", "1", "Use SQLite for player points and prefs.", _, true, 0.0, true, 1.0);
	g_cvarBlastPushing 			= CreateConVar("dtk_blastpushing", "2", "Control blast force pushing. 0 = off, 1 = unregulated, 2 = reduced", _, true, 0.0, true, 2.0);
	g_cvarBuildings 			= CreateConVar("dtk_buildings", "12", "Restrict engineer buildings. Add up these values to make the convar value: Sentry = 1, Dispenser = 2, Tele Entrance = 4, Tele Exit = 8", _, true, 0.0, false, 15.0);
	g_cvarAllowEurekaEffect 	= CreateConVar("dtk_allow_eurekaeffect", "0", "Allow the Eureka effect. 1 = yes, 0 = no.", _, true, 0.0, true, 1.0);
	g_cvarShowRoundStartMessage = CreateConVar("dtk_roundstartmessages", "1", "Show the round start messages. 1 = yes, 0 = no.", _, true, 0.0, true, 1.0);
	g_cvarUseActivatorHealthBar	= CreateConVar("dtk_activatorhealthbar", "1", "Use a boss health bar for the activator's health when they are damaged by an enemy. 1 = yes, 0 = no.", _, true, 0.0, true, 1.0);
...
public void OnClientAuthorized(int client, const char[] auth)
{
	...
	if (g_cvarUseSQL.BoolValue && StrEqual(auth, "BOT", false) || StrEqual(auth, "STEAMID_AUTH_LAN", false))
	{
		PrintToServer("  [DR] %N is a bot or on a LAN. We'll let them keep the default settings.", client);
		return;
	}
	
	if (g_cvarUseSQL.BoolValue && g_db != INVALID_HANDLE)		// if the database is connected
	{
...
public void OnClientDisconnect(int client)
{
	...
	if (g_cvarUseSQL.BoolValue && g_db != INVALID_HANDLE)
	{
		char auth[64];
...
{
	/*/ Reset array index queue points and prefs
...
void Event_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (g_cvarShowRoundStartMessage.BoolValue)
...
public Action TF2Items_OnGiveNamedItem(int client, char[] classname, int itemDefIndex, Handle &hItem)
{
	...
	if (itemDefIndex == 589 && !g_cvarAllowEurekaEffect.BoolValue)	// Eureka Effect
	{
	...
	if (g_cvarChatKillFeed)
		if (event.GetInt("attacker") == 0)	// Killed by world
		{
			...
			if (classname[0] != 'w')
			{
				...
				CPrintToChat(victim, "%t %t", "prefix", "killed by entity classname", classname, targetname);
			}
		}
...
{
	if (g_cvarUseActivatorHealthBar.BoolValue)
	{
	...
	if (TF2_GetClientTeam(victim) == TFTeam_Red && attacker == victim)
	{
		switch (g_cvarBlastPushing.IntValue)
		{
			case 0:
				return Plugin_Continue;
			case 1:
...
CheckForPlayers()
{
	...
	if (g_bWatchForPlayerChanges && CountPlayers(view_as<int>(TFTeam_Blue), false) < desired_activators && CountPlayers(_, false) > desired_activators)
	{
	...
	if (g_bWatchForPlayerChanges && CountPlayers(view_as<int>(TFTeam_Blue), false) == desired_activators)
	{
	...
	// At round restart
	if (!g_bWatchForPlayerChanges && CountPlayers(_, false) > 1)
	{
...
SelectActivatorTurn()
{
	...
	while (GetTeamClientCount(view_as<int>(TFTeam_Blue)) < g_cvarActivators.IntValue && GetTeamClientCount(view_as<int>(TFTeam_Red)) > 1)
	{
		if (IsClientInGame(client) && TF2_GetClientTeam(client) == TFTeam_Red)
		{
			MovePlayerToTeam(client, TFTeam_Blue);
...
DisplayNextActivators()
{
	...
	if (players[0] != 0)
	{
		...
		CPrintToChatAll("%t %t", "prefix", "next activators", string1, string2, string3);
	}
}
...
void CheckGameState()
{
	...
	if (CountPlayers(view_as<int>(TFTeam_Red), true) <= g_cvarTeamPushOnRemaining.IntValue && g_cvarTeamPushOnRemaining.BoolValue != false && g_cvarServerTeamMatePush.BoolValue == false && g_bIsRoundActive)
	{
	...
	if (CountPlayers(view_as<int>(TFTeam_Red), true) <= g_cvarRedGlowRemaining.IntValue && g_cvarRedGlowRemaining.BoolValue != false && g_cvarRedGlow.BoolValue == false && g_bIsRoundActive)
	{
		g_bIsRedGlowActive = true;
...
Action CommandListener_Build(int client, const char[] command, int args)
{
	...
	*/
	...
	if (g_cvarBuildings.IntValue != 0)
	{
		char arg1[2], arg2[2];
		GetCmdArg(1, arg1, sizeof(arg1)), GetCmdArg(2, arg2, sizeof(arg2));
		int iArg1 = StringToInt(arg1), iArg2 = StringToInt(arg2);
		
		if (iArg1 == 0 && (g_cvarBuildings.IntValue & BUILDINGS_DISPENSER) == BUILDINGS_DISPENSER)
		{
			CPrintToChat(client, "%t Sorry, you aren't allowed to build dispensers", "prefix");
			return Plugin_Handled;
		}
		if (iArg1 == 1 && iArg2 == 0 && (g_cvarBuildings.IntValue & BUILDINGS_TELE_ENTRANCE) == BUILDINGS_TELE_ENTRANCE)
		{
			CPrintToChat(client, "%t Sorry, you aren't allowed to build teleporter entrances", "prefix");
			return Plugin_Handled;
		}
		if (iArg1 == 1 && iArg2 == 1 && (g_cvarBuildings.IntValue & BUILDINGS_TELE_EXIT) == BUILDINGS_TELE_EXIT)
		{
			CPrintToChat(client, "%t Sorry, you aren't allowed to build teleporter exits", "prefix");
			return Plugin_Handled;
		}
		if (iArg1 == 2 && (g_cvarBuildings.IntValue & BUILDINGS_SENTRY) == BUILDINGS_SENTRY)
		{
			CPrintToChat(client, "%t Sorry, you aren't allowed to build sentries", "prefix");
			return Plugin_Handled;
		}
		// build 3 is for backwards compatability of the deprecated build argument to build a tele exit
		if (iArg1 == 3 && (g_cvarBuildings.IntValue & BUILDINGS_TELE_EXIT) == BUILDINGS_TELE_EXIT && TF2_GetPlayerClass(client) == TFClass_Engineer)
		{
			CPrintToChat(client, "%t Sorry, you aren't allowed to build teleporter exits", "prefix");
			return Plugin_Handled;
		}
...
Action Command_ResetPoints(int client, int args)
{
	...
	if (g_cvarUsePointsSystem.BoolValue == true)
	{
		...
		{
			CReplyToCommand(client, "%t %t", "prefix_notice", "points cannot be reset");
...
Action Command_English(int client, int args)
{
	...
	if ((g_iFlags[client] & PLAYERFLAGS_ENGLISH) == PLAYERFLAGS_ENGLISH)
	{
		...
		if (GetClientInfo(client, "cl_language", language, sizeof(language)))
		{
			...
			if (iLanguage == -1)
				CReplyToCommand(client, "%t When you next connect, your client language will be used", "prefix_notice");
			else
			{
				SetClientLanguage(client, iLanguage);
				CReplyToCommand(client, "%t Your client language of '%s' will now be used", "prefix_notice", language);
			}
		}
		return Plugin_Handled;
	}
	...
	{
		g_iFlags[client] |= PLAYERFLAGS_ENGLISH;
		CReplyToCommand(client, "%t Your language has been been set to English for this session", "prefix_notice");
		SetClientLanguage(client, 0);
	}
	return Plugin_Handled;
}
...
Action Command_PlayerData(int client, int args)
{
	...
	CReplyToCommand(client, "%t Check console for output", "prefix_notice");
...
Action AdminCommand_AwardPoints(int client, int args)
{
	...
	if (args != 2)
	{
		CReplyToCommand(client, "%t %t", "prefix", "wrong usage of award command");
		return Plugin_Handled;
	}
...
Action AdminCommand_ResetDatabase(int client, int args)
{
	...
	{
		CReplyToCommand(client, "%t To reset the database, supply the argument 'confirm'", "prefix_notice");
		return Plugin_Handled;
	}
	else if (args == 1 && StrEqual(confirm, "confirm"))
	{	
		CReplyToCommand(client, "%t Resetting the player database.", "prefix_notice");
		char query[255];
...
Action AdminCommand_ResetUser(int client, int args)
{
	...
	{
		CReplyToCommand(client, "%t Usage: sm_resetuser <user> confirm", "prefix_notice");
		return Plugin_Handled;
	}
	else if (args == 2 && StrEqual(confirm, "confirm"))
	{
		...
		{
			CReplyToCommand(client, "%t Deleting %N from the database...", "prefix_notice", target);
			char query[255];
