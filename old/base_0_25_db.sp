...
#define PLAYERFLAGS_NEW					(7)			// Connected, Preference and Fullpoints set to 1
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_prefs", Command_Preferences, "A shortcut command for setting your activator and points preference.");
	RegAdminCmd("sm_award", AdminCommand_AwardPoints, ADMFLAG_SLAY, "Award a peon some pity points.");
	RegAdminCmd("sm_resetdatabase", AdminCommand_ResetDatabase, ADMFLAG_CONVARS|ADMFLAG_CONFIG|ADMFLAG_RCON, "Reset the player database.");
	RegAdminCmd("sm_resetuser", AdminCommand_ResetUser, ADMFLAG_CONVARS|ADMFLAG_CONFIG|ADMFLAG_RCON, "Deletes a player's data from the database. They will be treated as a new player.");
}
	...
	if (g_bIsDeathrunMap)
	{
		...
		if (g_cvarUseSQL.BoolValue)
			...
			else
			{
				...
				char query[255];
				Format(query, sizeof(query), "CREATE TABLE IF NOT EXISTS %s (auth CHAR(64) UNIQUE, \
				name VARCHAR(64), points INT(32), flags INT(32), time INT(32) DEFAULT '0')", SQLITE_TABLE);
				SQL_FastQuery(g_db, query);
				
				// Remove entries older than a month, or 2629743 seconds
				Format(query, sizeof(query), "DELETE FROM %s WHERE time <= '%d'", SQLITE_TABLE, GetTime() - 2629743);
				SQL_FastQuery(g_db, query);
			}
	}
...
public void OnClientAuthorized(int client, const char[] auth)
{
	...
	if (g_cvarUseSQL.BoolValue && g_db != INVALID_HANDLE)		// if the database is connected
	{
		...
		char query[255];
		Format(query, sizeof(query), "SELECT points, flags from %s WHERE auth='%s'", SQLITE_TABLE, auth);
		...
		if (result != INVALID_HANDLE)
		{
			...
			{
				Format(query, sizeof(query), "INSERT INTO %s (auth, name, points, flags, time) \
				VALUES ('%s', '%N', '%d', '%d', '%d')", SQLITE_TABLE, auth, client, POINTS_STARTING, PLAYERFLAGS_NEW, GetTime());
				if (SQL_FastQuery(g_db, query))
				{
					PrintToServer("  [DR] %N is a new user. A new record has been created for them in the database using default values", client);
...
public void OnClientDisconnect(int client)
{
	...
	if (g_cvarUseSQL.BoolValue && g_db != INVALID_HANDLE)
	{
		...
		{
			char query[255];
			Format(query, sizeof(query), "UPDATE %s SET points='%d', flags='%d', time='%d' WHERE auth='%s'", SQLITE_TABLE, g_iPoints[client], g_iFlags[client], GetTime(), auth);
			
			if (SQL_FastQuery(g_db, query))
				PrintToServer("  [DR] Updated %N's data in the database. Points %d, flags %d, time %d, auth %s", client, g_iPoints[client], g_iFlags[client], GetTime(), auth);
			else
			{
...
Action AdminCommand_ResetDatabase(int client, int args)
{
	...
	{	
		...
		if (SQL_FastQuery(g_db, query))
			CReplyToCommand(client, "%t Player database has been reset. Changing the map will reinitialise it.", "prefix_notice");
		else
			CReplyToCommand(client, "%t Failed to reset the player database.", "prefix_notice");
	}
...
Action AdminCommand_ResetUser(int client, int args)
{
	...
	else if (args == 2 && StrEqual(confirm, "confirm"))
	{
		...
		else
		{
			...
			Format(query, sizeof(query), "DELETE from %s WHERE auth='%s'", SQLITE_TABLE, auth);
			if (SQL_FastQuery(g_db, query))
				CReplyToCommand(client, "%t %N has been deleted from the database", "prefix_notice", target);
			else
			{
				char error[255];
				SQL_GetError(g_db, error, sizeof(error));
				CReplyToCommand(client, "%t Failed to delete %N from the database. Check console for error message.", "prefix_notice", target);
