...
/*
	...
	20. Add a deathrun game mode toggle cvar so you can use it on non-DR maps.
	
	21. Review team pushing convar change hook and add a cvar to control the auto enabling at a set number of live players.
	
	22. Base points multiplier modifier.
...
/*
	...
	post_inventory_application won't set the 'no switch from melee' condition.
...
static int g_iPlayerData[33][3];	// Our new multi dimensional array for points

#define PLAYERDATA_INDEX 0

#define PLAYER_POINTS 0
#define PLAYER_MULTIPLIER 1
#define PLAYER_PREFERENCE 2

#define MULTIPLIER_FULL 2
#define MULTIPLIER_MODERATE 1
#define MULTIPLIER_NONE 0

#define PREFERENCE_NOTCONNECTED -1
#define PREFERENCE_NEVER 0
#define PREFERENCE_YESPLEASE 1

#define POINTS_STARTING 10 // 10
#define POINTS_AWARD 5 // 5
#define POINTS_CONSUMED 0
#define MINIMUM_ACTIVATORS 3
...
void Event_RoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (g_cvarUsePointsSystem.BoolValue == true)
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			...
			{
				int awarded = POINTS_AWARD * g_iPlayerData[i][PLAYER_MULTIPLIER];
				g_iPlayerData[i][PLAYER_POINTS] += awarded;
				PrintToChat(i, "%t You have received %d queue points for a total of %d", "prefix", awarded, g_iPlayerData[i][PLAYER_POINTS]);
				PrintToServer("  [DR] g_iPlayerData[%d][PLAYER_POINTS] %N has %d points.", i, i, g_iPlayerData[i][PLAYER_POINTS]);
				PrintToConsoleAll("  [DR] %N has %d points.", i, g_iPlayerData[i][PLAYER_POINTS]);
			}
		}
	}
...
void Event_PlayerConnect(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (g_cvarUsePointsSystem.BoolValue == true)
	{
		...
		g_iPlayerData[client][PLAYER_POINTS] = POINTS_STARTING;
		g_iPlayerData[client][PLAYER_MULTIPLIER] = MULTIPLIER_FULL;
		g_iPlayerData[client][PLAYER_PREFERENCE] = PREFERENCE_YESPLEASE;
		//PrintToServer("  [DR] Reset new player %N's queue points to %d.", client, POINTS_STARTING); // Client index doesn't resolve this early
	}
}
...
void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (g_bIsRedGlowActive || g_cvarRedGlow.BoolValue && TF2_GetClientTeam(client) == TFTeam_Red)
	{
...
// On player inventory application (only hooked on deathrun map)
Event_InventoryApplied(Event event, const char[] name, bool dontBroadcast)
{
	if (g_bIsDeathrunMap && g_cvarRedMeleeOnly.BoolValue)
	{
		int client = GetClientOfUserId(event.GetInt("userid"));
...
void Event_PlayerDisconnect(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (g_cvarUsePointsSystem.BoolValue == true)
	{
		...
		g_iPlayerData[client][PLAYER_MULTIPLIER] = MULTIPLIER_FULL;
		g_iPlayerData[client][PLAYER_PREFERENCE] = PREFERENCE_YESPLEASE;
		PrintToServer("  [DR] Reset disconnecting player %N's queue points to %d.", client, POINTS_STARTING);
	}
}
...
Action CheckForPlayers()
{
	...
	if (g_bWatchForPlayerChanges && CountPlayers(_:TFTeam_Blue, false) < desired_activators && CountPlayers(0, false) > desired_activators)
	{
		...
		RequestFrame(RedistributePlayers);		// Needed to do request frame to keep the red player count right
		//RedistributePlayers();				// Make some changes. We'll go back to watching when the changes are done.
...
void RedistributePlayers() // was public
{
	...
	// Use the new system for moving blue based on points
	if (g_cvarUsePointsSystem.BoolValue == true)
	{
		int prefers;
		...
		{
			if (g_iPlayerData[i][PLAYER_PREFERENCE] == PREFERENCE_YESPLEASE)
				prefers += 1;
		}
...
SelectActivatorPoints()
{
	...
	for (int i = 0; i < MaxClients; i++)
	{
		...
		points_order[i][1] = g_iPlayerData[i + 1][PLAYER_POINTS];
	}
	...
	while (GetTeamClientCount(_:TFTeam_Blue) < g_cvarActivators.IntValue && GetTeamClientCount(_:TFTeam_Red) > 1)
	{
		...
		{
			MovePlayerToTeam(client, _:TFTeam_Blue);
			g_iPlayerData[client][PLAYER_POINTS] = POINTS_CONSUMED;
...
/*Action Timer_TellPoints(Handle timer)
{
	...
	{
		if (IsClientInGame(i) && TF2_GetClientTeam(i) == TFTeam_Red)
			PrintToChat(i, "%t You are currently sitting on %d queue points. Rejoice.", "prefix", g_iPlayerData[i][PLAYER_POINTS]);
	}
}*/
...
void CheckGameState()
{
	...
	if (CountPlayers(_:TFTeam_Red, true) <= g_cvarRedGlowRemaining.IntValue && g_cvarRedGlowRemaining.BoolValue != false && g_cvarRedGlow.BoolValue == false && g_bIsRoundActive)
	{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			...
			{
				SetEntProp(i, Prop_Send, "m_bGlowEnabled", 1);
				PrintToServer("  [DR] CountPlayer has determined that %N should glow.", i);
			}
		}
...
// Set a player to switch to their melee weapon and deletes their other weapons
...
Action Command_ShowPoints(int client, int args)
{
	...
	if (g_cvarUsePointsSystem.BoolValue == true)
		ReplyToCommand(client, "%t You have %d points.", "prefix", g_iPlayerData[client][PLAYER_POINTS]);
	else
...
Action Command_ResetPoints(int client, int args)
{
	...
	{
		g_iPlayerData[client][PLAYER_POINTS] = POINTS_CONSUMED;
		ReplyToCommand(client, "%t Your points have been reset to %d.", "prefix", g_iPlayerData[client][PLAYER_POINTS]);
...
// Select my points gain modifier preference
...
*/
	...
	if (args < 1)
	{
		ReplyToCommand(client, "%t Your current preference is to receive %d points per round.", "prefix", g_iPlayerData[client][PLAYER_MULTIPLIER] * POINTS_AWARD);
		return Plugin_Handled;
	}
	...
	switch (selection)
	{
		case 0:
		{
			g_iPlayerData[client][PLAYER_MULTIPLIER] = MULTIPLIER_NONE;
			g_iPlayerData[client][PLAYER_PREFERENCE] = PREFERENCE_NEVER;
			ReplyToCommand(client, "%t You will no longer receive any queue points.", "prefix");
		}
		case 1:
		{
			g_iPlayerData[client][PLAYER_MULTIPLIER] = MULTIPLIER_MODERATE;
			g_iPlayerData[client][PLAYER_PREFERENCE] = PREFERENCE_YESPLEASE;
			ReplyToCommand(client, "%t You will now receive fewer queue points.", "prefix");
		}
		case 2:
		{
			g_iPlayerData[client][PLAYER_MULTIPLIER] = MULTIPLIER_FULL;
			g_iPlayerData[client][PLAYER_PREFERENCE] = PREFERENCE_YESPLEASE;
			ReplyToCommand(client, "%t You will receive the maximum amount of queue points.", "prefix");
		}
	}
...
Action Command_GivePointsAdmin(int client, int args)
{
	...
	{
		g_iPlayerData[target][PLAYER_POINTS] += points;
		ReplyToCommand(client, "%t You have awarded %N %d queue points, making their total %d.", "prefix", target, points, g_iPlayerData[target][PLAYER_POINTS]);
		PrintToChat(target, "%t %N has awarded you %d points, making your total %d.", "prefix", client, points, g_iPlayerData[target][PLAYER_POINTS]);
		return Plugin_Handled;
	}
}
