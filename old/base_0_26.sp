...
/*
	...
	46. Allow /award to give to multiple clients
	
	47. Make SQL use toggleable?
	
	48. Add plugin version number to menu.
	
	49. Show current preference in menu.
	
	50. Plugin information menu option that leads to a group post.
	
	51. Review bit flags. See if it's possible to only transfer the first half or something.
	
	52. Alternative phrases for points consumption.
	
	53. Add a 'change flag' command. Sends a query to the DB and changes a flag to on or off. Can be used for a particular user, maybe a @group, or for everyone.
...
#define PLAYERFLAGS_NEW					(PLAYERFLAG_PREFERENCE|PLAYERFLAG_FULLPOINTS)
	...
	if (g_cvarUseSQL.BoolValue && g_db != INVALID_HANDLE)		// if the database is connected
	{
		...
		if (result != INVALID_HANDLE)
		{
			if (SQL_FetchRow(result))	// If a row was fetched
			{
				...
				g_iFlags[client] = result.FetchInt(field);
				PrintToServer("  [DR] Query of %N returned: Points %d, flags: %d", client, g_iPoints[client], g_iFlags[client]);
			}
...
public void OnClientDisconnect(int client)
{
	...
	if (g_cvarUseSQL.BoolValue && g_db != INVALID_HANDLE)
	{
		...
		{
			char query[255];
			Format(query, sizeof(query), "UPDATE %s SET points=%d, flags=%d, last_seen=%d WHERE steamid=%d", SQLITE_TABLE, g_iPoints[client], g_iFlags[client], GetTime(), GetSteamAccountID(client));
			
			if (SQL_FastQuery(g_db, query))
				PrintToServer("  [DR] Updated %N's data in the database. Points %d, flags %d, last_seen %d, steamid %d", client, g_iPoints[client], g_iFlags[client], GetTime(), GetSteamAccountID(client));
			else
			{
...
{
	if (victim != attacker || TF2_GetClientTeam(victim) != TFTeam_Red)
		return Plugin_Continue;
...
CheckForPlayers()
{
	...
	if (!g_bWatchForPlayerChanges && CountPlayers(_, false) > 1)
	{
		...
		RequestFrame(RedistributePlayers);
			// Request frame added to preclude team numbers becoming wrong from switching too quickly
